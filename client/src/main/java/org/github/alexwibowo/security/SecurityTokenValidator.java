...
public class SecurityTokenValidator {
    ...
    /**
     * Verify the message digest in {@link SecurityToken#messageDigest}, using
     * <ul>
     *     <li>{@link SecurityToken#timestamp}</li>
     *     <li>{@link SecurityToken#nonce}</li>
     ...
     * @throws SecurityException on failure to verify the digest
     */
    ...
    private void verifyTimestampFreshness(SecurityToken digest, long ttlInSeconds) throws SecurityException {
        try {
            ...
            if ((nowMillis - timestampMillis) > ttlInMilliseconds) {
                throw new SecurityException(String.format("Timestamp is older than %d seconds", ttlInSeconds));
            }
